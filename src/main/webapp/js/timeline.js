				...
				$.each(res, function(index, value){
					...
					if(value.board_photo==null) {
						source='<a href="timelineDetail.do?board_num='+value.board_num+'" class="link_detail">'+
							   ...
							   '<div class="board_subject">'+value.board_subject+'</div>'+
							   '<div class="board_writer">by '+value.user_nickname+'</div></div></a>';
					} else if(value.board_photo.indexOf('http')>0) {
						source='<a href="timelineDetail.do?board_num='+value.board_num+'" class="link_detail">'+
						   	   '<div class="board_grid" style="background: linear-gradient( rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.3)), url('+value.board_photo+'); background-position: center; background-repeat: no-repeat; background-size: cover;" >'+
						   	   '<div class="board_subject">'+value.board_subject+'</div>'+
							   '<div class="board_writer">by '+value.user_nickname+'</div></div></a>';
					} else {
						source='<a href="timelineDetail.do?board_num='+value.board_num+'" class="link_detail">'+
